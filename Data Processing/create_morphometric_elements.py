...
def spatial_distribution(building, tessel, street, sw1, sw3):
    logging.info("Spatial Distribution...")
    building_gdf = building
    tessel_gdf = tessel[["bID", "geometry"]]
    street_gdf = street[["sID", "geometry"]]

    ## Distance Band - Cocok untuk hubungan spasial yang bersifat continues
    logging.info("Create distance spatial weights...")

    dist1 = libpysal.weights.DistanceBand.from_dataframe(building_gdf, 50, ids="bID")
    dist2 = libpysal.weights.DistanceBand.from_dataframe(building_gdf, 150, ids="bID")
    dist3 = libpysal.weights.DistanceBand.from_dataframe(building_gdf, 300, ids="bID")
    ...
    building_gdf["b_neighbor_50"] = mm.Neighbors(
        building_gdf, dist1, "bID", verbose=False
    ).series
    building_gdf["b_neighbor_150"] = mm.Neighbors(
        building_gdf, dist2, "bID", verbose=False
    ).series
    building_gdf["b_neighbor_300"] = mm.Neighbors(
        building_gdf, dist3, "bID", verbose=False
    ).series
...
def calculate(kec: str):
    ...
    buildings_gdf = spatial_distribution(buildings_gdf, tessel_gdf, street_gdf, sw1, sw3)
